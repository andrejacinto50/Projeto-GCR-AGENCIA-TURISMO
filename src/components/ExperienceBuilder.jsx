...
import { MessageCircle, Sparkles, Users, CalendarDays } from 'lucide-react'
...
const interesses = [
  {
    ...
    motivo: 'ideal para conhecer ilhas, praias e cenários clássicos de Governador Celso Ramos.',
  },
  {
    nome: 'Natureza',
    recomendado: 'Trilhas Guiadas',
    alternativa: 'City Tour',
    motivo: 'perfeito para mirantes, contato com a Mata Atlântica e paisagens mais tranquilas.',
  },
  {
    nome: 'Família',
    recomendado: 'Passeio de Escuna',
    alternativa: 'Passeio de Lancha',
    motivo: 'uma opção confortável, visual e fácil de aproveitar em grupo.',
  },
  {
    nome: 'Aventura',
    recomendado: 'Voo de Parapente',
    alternativa: 'Passeio de Buggy',
    motivo: 'indicado para quem quer uma experiência mais marcante e cheia de emoção.',
  },
  {
    nome: 'Pesca',
    recomendado: 'Pesca Embarcada',
    alternativa: 'Passeio de Lancha',
    motivo: 'pensado para quem quer viver o mar com mais tempo, calma e orientação local.',
  },
]
...
export default function ExperienceBuilder() {
  ...
  const mensagem = useMemo(() => {
    ...
    return [
      ...
      dataFormatada ? `Data pretendida: ${dataFormatada}` : `Data pretendida: ainda quero confirmar`,
      ``,
  ...
  }, [escolha, quantidade, perfil, dataFormatada])

  const podeEnviar = Boolean(escolha)
    ...
    <section className="builder-premium">
      ...
      <div className="builder-content">
        <div className="builder-copy">
          ...
          <h3>Monte sua experiência e receba recomendações personalizadas</h3>

          <p>
            Escolha o estilo do passeio, informe a quantidade de pessoas e a data
            pretendida. A GCR recebe tudo organizado no WhatsApp para indicar o
            melhor roteiro.
          </p>
        </div>
        ...
        <div className="builder-preview-card">
          ...
          {escolha ? (
            ...
            <p>
              Selecione um estilo de passeio para gerar uma sugestão personalizada.
            </p>
          )}
        </div>
      </div>
          ...
          <div className="builder-options">
            {interesses.map((item) => (
              <button
                type="button"
                key={item.nome}
                onClick={() => setInteresse(item.nome)}
                className={`builder-option ${
                  interesse === item.nome ? 'builder-option-active' : ''
                }`}
              >
                {item.nome}
              </button>
      ...
      <div className="builder-actions">
        <a
          ...
          className={`builder-submit ${
            podeEnviar ? 'builder-submit-active' : 'builder-submit-disabled'
          }`}
        >
        ...
        {!podeEnviar && (
          <p>Selecione um tipo de passeio para liberar o WhatsApp.</p>
        )}
      </div>
    </section>
  )
